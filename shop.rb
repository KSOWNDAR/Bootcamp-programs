...
def cash_box_debit(balance_cost,balance_amount_list,order_count,cost_price_list,given_amount_list,duplicate_cash_box)
    ...
    while balance_cost > 0
        ...
        chocolate_given = false 
        while((value_of_note < cashes.length) && ((balance_cost/cashes[value_of_note]).abs <= 0))
            value_of_note += 1
        end 
        index = (cashes[value_of_note]).to_s
        while((value_of_note < cashes.length) && ($cash_box[index]<=0))
            if ((index.eql?("10")) || (index.eql?("5")) || (index.eql?("2")) || (index.eql?("1")))
                index += "c"
                if $cash_box[index] == 0 
                 index = index[0..-2]
                else
        ...
        if value_of_note == cashes.length
            if(balance_cost <= 5)
...
$cash_box ={'1' => 0,'1c' => 0,'2' => 0,'2c' => 0,'5' => 0,'5c' => 0,'10' => 0,'10c' => 0,'20' => 2,'50' => 1,'100' => 0,'200' => 1,'500' => 0,'2000' => 0}
open_shop